  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        ...
        <Link href="/studio/products/new" className="inline-flex items-center gap-2 rounded-full bg-foreground px-5 py-2.5 text-xs uppercase tracking-[0.06em] text-background hover:bg-foreground/85 transition-colors">
      ...
      <div className="mt-8 space-y-3">
        {products.map((p) => (
          <div key={p.slug} className="flex items-center gap-4 rounded-2xl border border-border p-4">
            ...
            <span className="shrink-0 rounded-full bg-[color:var(--paper)] px-3 py-1 text-[10px] uppercase tracking-[0.06em] text-muted-foreground">
              {p.status}
            </span>
