...
import { getBanner, getSizeGuide, getHandoverGuide, getIsolationMode } from "@/lib/settings-server";
...
export default async function SettingsPage() {
  ...
  const [banner, sizeGuide, handoverGuide, isolation] = await Promise.all([
    getBanner(),
    getSizeGuide(),
    getHandoverGuide(),
    getIsolationMode(),
  ...
  return (
    <div>
      ...
      <div className="mt-8 max-w-lg">
        <SettingsClient banner={banner} sizeGuide={sizeGuide} handoverGuide={handoverGuide} isolation={isolation} />
      </div>
    </div>
  );
}
